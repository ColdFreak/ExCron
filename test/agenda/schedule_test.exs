defmodule Agenda.ScheduleTest do
  ...
  @all_times %Schedule{minute: Enum.to_list(0..59), hour: Enum.to_list(0..23), day_of_month: Enum.to_list(1..31), month: Enum.to_list(1..12), day_of_week: Enum.to_list(0..6)}

  test "whether or not a schedule include a given time" do
    good_times = [
      {{2015, 5, 5}, {23, 53, 42}}
    ]
    bad_times = [
  ...
  def assert_schedule_time_spec_matches(schedule, good_times, bad_times)do
    Enum.each(good_times, fn(time) -> 
      assert Schedule.include?(schedule, time) end)

    Enum.each(bad_times, fn(time) ->
      refute Schedule.include?(schedule, time) end)
  end
